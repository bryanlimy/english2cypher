...
def get_args(extend=lambda a: None):
  ...
  parser.add_argument(
      ...
      type=int,
      default=None,
      help="Limit number of data points, to quickly test code")
  ...
  parser.add_argument(
      '--quick',
      action='store_true',
      help="Compromise model quality for training speed")
  ...
  for i in [*args["modes"], "all"]:
    ...
    args[i + "_src_path"] = os.path.join(args["input_dir"],
                                         i + "_" + args["src_filename"])
    args[i + "_tgt_path"] = os.path.join(args["input_dir"],
                                         i + "_" + args["tgt_filename"])
