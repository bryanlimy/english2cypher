...
def load_vocab_hashtable(args):
  return tf.contrib.lookup.index_table_from_file(
      args["vocab_path"], vocab_size=args["vocab_size"], default_value=UNK_ID)


def load_inverse_vocab_hashtable(args):
  return tf.contrib.lookup.index_to_string_table_from_file(
      args["vocab_path"], vocab_size=args["vocab_size"], default_value=UNK)
...
def StringDataset(s):
  ...
  return tf.data.Dataset.from_generator(generator, tf.string,
                                        tf.TensorShape([]))
...
def gen_input_fn(args, mode, question=None):
  ...
  src_dataset = src_dataset.map(
      lambda l: tf.cast(vocab_hashtable.lookup(l), tf.int32))
  ...
  d = d.map(lambda src, tgt_in, tgt_out: ({
      ...
      "tgt_out": tgt_out,
      "src_len": tf.size(src),
      "tgt_len": tf.size(tgt_in),}, tgt_out))
