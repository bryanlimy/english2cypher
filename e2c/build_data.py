...
def download_gqa(args):
  ...
  assert args["gqa_path"][0:len(args["input_dir"])] == args[
      "input_dir"], "GQA path must be inside input-dir for automatic download"

  if not tf.gfile.Exists(args["gqa_path"]):
    zip_path = "./gqa.zip"
    print("Downloading gqa.yaml (61mb)")
    urllib.request.urlretrieve(
        "https://storage.googleapis.com/octavian-static/download/english2cypher/gqa.zip",
        zip_path)
