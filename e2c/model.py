...
def basic_cell(args, i, unit_mul):
  ...
  c = tf.contrib.rnn.LSTMCell(args['num_units'] * unit_mul,
...
def model_fn(features, labels, mode, params):
  ...
  word_embedding = tf.get_variable(
      "word_embedding", [args["vocab_size"], args["num_units"]], tf.float32)
  ...
  (fw_output, bw_output), (fw_states,
                           bw_states) = tf.nn.bidirectional_dynamic_rnn(
                               fw_cell,
                               bw_cell,
                               src,
                               dtype=dtype,
  ...
  output_layer = tf.layers.Dense(
      args["vocab_size"], use_bias=False, name="output_dense")
  ...
  if mode in [
    ...
    tgt_in = tf.nn.embedding_lookup(word_embedding,
                                    tf.transpose(features["tgt_in"]))

    decoder_helper = tf.contrib.seq2seq.TrainingHelper(
        tgt_in, features["tgt_len"], time_major=time_major)

    with tf.variable_scope("decoder", reuse=tf.AUTO_REUSE) as decoder_scope:

      d_cell, d_cell_initial = decoder_cell(args, 2, None, dynamic_batch_size,
                                            features, encoder_outputs,
                                            encoder_state)

      guided_decoder = tf.contrib.seq2seq.BasicDecoder(d_cell, decoder_helper,
                                                       d_cell_initial)
  ...
  if mode in [tf.estimator.ModeKeys.PREDICT, tf.estimator.ModeKeys.EVAL]:
    ...
    with tf.variable_scope("decoder", reuse=tf.AUTO_REUSE) as decoder_scope:
      ...
      p_cell, p_cell_initial = decoder_cell(args, 2, args["beam_width"],
                                            dynamic_batch_size, features,
                                            encoder_outputs, encoder_state)
  ...
  if mode in [tf.estimator.ModeKeys.TRAIN, tf.estimator.ModeKeys.EVAL]:
    ...
    loss = tf.reduce_sum(
        crossent * target_weights) / tf.to_float(dynamic_batch_size)
  ...
  if mode == tf.estimator.ModeKeys.EVAL:
    ...
    def pad_to_seq_len(this, be_like):
      ...
      return tf.pad(
          this, [[0, delta], *no_padding],
          constant_values=tf.cast(vocab_const['tgt_eos_id'], this.dtype))

    def tile_to_beam(v):
      return tf.tile(tf.expand_dims(v, -1), [1, 1, args["beam_width"]])

    beam_weights = tile_to_beam(
        pad_to_seq_len(tf.cast(target_weights, tf.int32), beam_predictions))
    ...
    beam_cmp = tf.equal(beam_labels * beam_weights,
                        beam_padded_predictions * beam_weights)
    ...
    eval_metric_ops = {
        ...
        "beam_strict_accuracy":
        tf.metrics.mean(beam_strict_cmp_mean),
        "beam_mode_accuracy":
        tf.metrics.mean(beam_mode_cmp_mean)
    }

    eval_metric_ops_ext = {**eval_metric_ops}
    eval_metric_ops_ext["loss"] = tf.metrics.mean(loss)

    eval_hooks = [FloydHubMetricHook(eval_metric_ops_ext, prefix="eval_")]
  ...
  if mode == tf.estimator.ModeKeys.PREDICT:
    ...
    predictions = {
        "input":
        vocab_inverse.lookup(tf.to_int64(features["src"])),
        "target":
        vocab_inverse.lookup(tf.to_int64(features["tgt_out"])),
        "guided":
        vocab_inverse.lookup(tf.to_int64(tf.transpose(guided_predictions))),
        "beam":
        vocab_inverse.lookup(
            tf.to_int64(tf.transpose(beam_predictions, [1, 2, 0])))
    }
